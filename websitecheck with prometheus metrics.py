...
popup_url = "http://popup.example.com"
...
mail_to = ["noc@example.com", "other.user@example.com"]
email_username = "noc@example.com"
email_password = "strongPassword"

# powershell
powershell_exe_file_name = "powershell.exe"
powershell_exe_file_path = r"C:\Windows\System32\WindowsPowerShell\v1.0"
...
if not website_status_code_check(main_url):
    ...
    print("main url inaccessible. delete ok html, send email, create err html")
    ...
    #  main url is  available but the popup window doesnt return code 200
    ...
    html_message = f"""html message"""
    create_update_html_file(html_path_name, html_message)
    mail_subject_arg = "popup url is not accessible!"
    mail_body_arg = f"""email message """
    email_send(mail_subject_arg, mail_body_arg)
else:
    ...
    for i in range(len(user_list)):
        user = user_list[i]
        main_time = main_loading_time_list[i]
        popup_time = popup_loading_time_list[i]
        result.append(f'main_url_loading_time{{user="{user}"}} {main_time}')
        result.append(f'popup_url_loading_time{{user="{user}"}} {popup_time}')

    # write to metrics.txt file
    with open('metrics.txt', 'w') as metrics:
        for line in result:
            metrics.write(line + '\n')
    subprocess.call(["python", to_prometheus_script_path])
    ...
    if (any(x >= main_url_upper_limit_seconds_for_service_restart for x in main_loading_time_list)) or \
        ...
        html_message = f"""html message"""
        create_update_html_file(html_path_name, html_message)
        mail_subject_arg = f" {title} loading time is above service restart limit!"
        mail_body_arg = f"""email message"""
        email_send(mail_subject_arg, mail_body_arg)
    ...
    # start checking if ANY element of ANY list is above email notification limit
        ...
        html_message = f"""html message"""
        create_update_html_file(html_path_name, html_message)
        mail_subject_arg = f"{title} loading time is above warning limit!"
        mail_body_arg = f"""email message"""
        email_send(mail_subject_arg, mail_body_arg)
    # else all is good
    else:
